...
本页面介绍如何在LangChain中使用Replicate运行模型。
...
只有`model`参数是必需的，但是其他任何模型参数也可以通过`input={model_param: value, ...}`的格式传递。

例如，如果我们正在运行稳定的扩散，并且想要更改图像的尺寸：

```
Replicate(model="stability-ai/stable-diffusion:db21e45d3f7023abc2a46ee38a23973f6dce16bb082a930b0c49861f96d1e5bf", input={'image_dimensions': '512x512'})
```

*请注意，模型的输出结果只会返回第一个结果*。
从这里开始，我们可以初始化我们的模型：
...
我们可以使用这种语法调用任何Replicate模型（不仅限于LLMs）。例如，我们可以调用[Stable Diffusion](https://replicate.com/stability-ai/stable-diffusion)模型：
