...
## 顺序链
当然，并不是所有的顺序链都像将一个字符串作为参数传递，并为链中的所有步骤获取一个字符串作为输出那样简单。在下一个示例中，我们将尝试更复杂的链式结构，涉及多个输入，并且还有多个最终输出。 

特别重要的是我们如何命名输入/输出变量名。在上面的示例中，我们不需要考虑这个问题，因为我们只是将一个链的输出直接作为下一个链的输入传递，但在这里，我们必须考虑这个问题，因为我们有多个输入。
...
### 顺序链中的记忆
有时您可能希望在链的每一步或链的后续部分传递一些上下文，但是维护和链接输入/输出变量可能会变得混乱。使用`SimpleMemory`是一种方便的方法，可以管理这一点并简化您的链条。

例如，使用前面的playwright SequentialChain，假设您想要包含有关剧本的日期、时间和位置的一些上下文，并使用生成的简介和评论创建一些社交媒体帖子文本。您可以将这些新的上下文变量添加为`input_variables`，或者我们可以添加一个`SimpleMemory`到链中来管理这个上下文。
