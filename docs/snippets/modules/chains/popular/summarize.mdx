## 准备数据
首先，我们准备数据。在这个例子中，我们从一个长文档中创建多个文档，但是这些文档可以以任何方式获取（这个笔记本的重点是强调在获取文档之后要做什么）。
...
如果您想对发生的情况有更多的控制和理解，请参阅下面的信息。

## `stuff` 链

本部分展示了使用 `stuff` 链进行摘要的结果。
...
您也可以在该链中使用自定义提示。在这个例子中，我们将用意大利语进行回复。
...
这一部分展示了使用 `map_reduce` 链进行摘要生成的结果。
...
我们还可以返回`map_reduce`链的中间步骤，以便进行检查。这可以通过`return_map_steps`变量实现。
...
您也可以在此链中使用自己的提示。在这个例子中，我们将用意大利语回复。
...
您也可以使用多个输入提示。在这个例子中，我们将使用一个MapReduce链来回答关于我们的代码的特定问题。
...
我们还可以返回`refine`链的中间步骤，以供检查。这可以通过设置`return_refine_steps`变量来实现。
...
您还可以使用自己的提示来进行对话。在这个例子中，我们将用意大利语回复。
