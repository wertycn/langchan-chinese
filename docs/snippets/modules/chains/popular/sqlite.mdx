在内部，LangChain使用SQLAlchemy来连接SQL数据库。`SQLDatabaseChain`可以与SQLAlchemy支持的任何SQL方言一起使用，例如MS SQL、MySQL、MariaDB、PostgreSQL、Oracle SQL、[Databricks](../../../ecosystem/integrations/databricks.html)和SQLite。有关连接到数据库的要求的更多信息，请参阅SQLAlchemy文档。例如，连接到MySQL需要适当的连接器，如PyMySQL。MySQL连接的URI可能类似于：`mysql+pymysql://user:pass@some_mysql_db_address/db_name`。

这个演示使用SQLite和示例的Chinook数据库。
要设置它，请按照https://database.guide/2-sample-databases-sqlite/上的说明进行操作，将`.db`文件放置在此存储库根目录下的notebooks文件夹中。
...
**注意：** 对于数据敏感的项目，您可以在`SQLDatabaseChain`的初始化中指定`return_direct=True`，以直接返回SQL查询的输出，而无需进行任何额外的格式化。这样可以防止LLM看到数据库中的任何内容。但需要注意的是，默认情况下，LLM仍然可以访问数据库的模式（即方言、表和键名称）。
...
## 使用查询检查器
有时候语言模型会生成一些带有小错误的无效SQL语句，可以使用SQL数据库代理使用的相同技术来自动纠正这些错误。您只需在创建链时指定此选项：
...
## 自定义提示符
您还可以自定义使用的提示符。下面是一个示例，提示它将foobar视为Employee表的同义词。
...
您还可以返回SQLDatabaseChain的中间步骤。这样可以让您访问生成的SQL语句，以及对SQL数据库执行该语句的结果。
...
## 选择如何限制返回的行数
如果您要查询表的多行数据，可以使用'top_k'参数来选择要获取的最大结果数（默认为10）。这对于避免超过提示的最大长度或不必要地消耗令牌的查询结果非常有用。
...
## 添加每个表的示例行
有时，数据的格式并不明显，最好在提示中包含表格中的几行样本，以便LLM在提供最终查询之前了解数据。在这里，我们将使用这个功能，通过提供`Track`表中的两行来让LLM知道艺术家的全名已保存。
...
### 自定义表格信息
在某些情况下，提供自定义表格信息可能比使用自动生成的表格定义和第一个`sample_rows_in_table_info`样本行更有用。例如，如果您知道表格的前几行没有信息量，手动提供更多样化或提供更多信息的示例行可能会有帮助。如果存在不必要的列，还可以限制模型可见的列。 

可以将此信息作为字典提供，其中表名作为键，表信息作为值。例如，让我们为Track表提供一个自定义定义和示例行，只包含几个列：
...
请注意，我们自定义的 `Track` 表的定义和示例行覆盖了 `sample_rows_in_table_info` 参数。在这个例子中没有被 `custom_table_info` 覆盖的表，比如 `Playlist`，将会像往常一样自动收集它们的表信息。
...
该链的步骤如下：
    ...
    1. 根据查询确定要使用哪些表。
    2. 基于这些表，调用普通的SQL数据库链。

在数据库中表的数量很大的情况下，这是非常有用的。
...
有时候，您可能无法使用OpenAI或其他托管大型语言模型的服务。当然，您可以尝试使用`SQLDatabaseChain`与本地模型，但很快就会意识到，即使使用大型GPU在本地运行大多数模型，也很难生成正确的输出。
...
只要按照上面指定的方式使用查询检查器，这个模型应该可以处理非常简单的SQL查询，例如：
...
即使是这个相对较大的模型，很可能无法单独生成更复杂的SQL。但是，您可以记录其输入和输出，以便手动校正它们，并将校正后的示例用于后续的few shot prompt示例。在实践中，您可以记录任何引发异常的链的执行（如下面的示例所示），或者在结果不正确的情况下（但没有引发异常）直接获得用户反馈。
...
多次运行上述代码片段，或在部署的环境中记录异常，以收集大量由您的语言模型生成的输入、table_info和sql_cmd的示例。sql_cmd的值可能是不正确的，您可以手动修复它们以建立一系列示例，例如，我们在这里使用YAML来保持我们的输入和已纠正的SQL输出的整洁记录，以便随着时间的推移逐步构建。
...
现在您已经有了一些示例（带有手动更正的输出SQL），您可以按照通常的方式进行少量提示种子。
...
现在，模型应该在这个几个示例输入的情况下表现得更好，特别是对于与您提供的示例类似的输入。
