## 准备数据
首先我们准备数据。对于这个例子，我们在一个向量数据库上进行相似性搜索，但这些文档可以以任何方式获取（本文档的重点是在获取文档之后要做什么）。
...
如果您想更加掌控和了解正在发生的事情，请参阅下面的信息。

## `stuff` Chain

本部分展示了使用`stuff` Chain进行问答的结果。
...
您还可以使用自己的提示语来使用这个链条。在这个示例中，我们将以意大利语回复。
...
我们还可以返回`map_reduce`链的中间步骤，以便进行检查。这可以通过`return_map_steps`变量来实现。
...
您也可以使用自己的提示来使用该链。在这个例子中，我们将用意大利语回答。
...
在使用`map_reduce`链时，要注意在映射步骤中使用的批处理大小。如果批处理大小过大，可能会导致速率限制错误。您可以通过设置所使用的LLM的批处理大小来控制此参数。请注意，此参数仅适用于具有此参数的LLM。以下是设置批处理大小的示例:
...
我们还可以返回`refine`链的中间步骤，以便进行检查。这可以通过`return_refine_steps`变量来实现。
...
您还可以使用自己的提示来操作此链条。在这个例子中，我们将以意大利语回复。
...
这个部分展示了使用 `map-rerank` 链来进行带有来源的问答的结果。
...
您也可以在这个链条中使用自己的提示。在这个例子中，我们将用意大利语回答。
